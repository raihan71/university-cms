...
<div class="container-fluid mb-4">
  ...
  <h1 class="mt-4">Edit Pengumuman/Berita</h1>
  ...
  <div class="card">
    <div class="card-body">
        <form action="{{ route('portal-admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          <div class="form-group">
              <label for="title">Judul Pengumuman/Berita</label>
              <input autocomplete="off" type="text" class="form-control" id="title" name="title" value="{{ $news->title }}" required>
          </div>
          <div class="form-group">
          ...
          <a href="{{ route('portal-admin.news.index') }}" class="btn btn-secondary ml-2">Batal</a>
      </form>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    ...
    const trixInitialValues = {
        content: {!! json_encode(old('content', $news->content)) !!},
    };
