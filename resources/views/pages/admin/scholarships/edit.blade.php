...
<div class="container-fluid mb-4">
  ...
  <h1 class="mt-4">Edit Beasiswa</h1>
  <ol class="breadcrumb mb-4">
      <li class="breadcrumb-item"><a href="{{ route('portal-admin.scholarships.index') }}">Daftar Beasiswa</a></li>
      <li class="breadcrumb-item active">Edit Beasiswa</li>
      <li class="breadcrumb-item active">{{ $scholarship->name }}</li>
  </ol>
  <div class="card">
    <div class="card-body">
        <form action="{{ route('portal-admin.scholarships.update', $scholarship->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          <div class="form-group">
              <label for="name">Nama</label>
              <input autocomplete="off" type="text" class="form-control" id="name" name="name" value="{{ $scholarship->name }}" required>
          </div>
          <div class="form-group">
              <label for="description">Deskripsi</label>
              <textarea class="form-control" id="description" name="description" rows="3" required>{{ $scholarship->description }}</textarea>
          </div>
          <div class="card mb-3">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" placeholder="Contoh: https://example.com atau konseling (tanpa link, kecil)" id="link" name="link" value="{{ $scholarship->link }}">
          </div>
          <button type="submit" class="btn btn-primary">Update</button>
          <a href="{{ route('portal-admin.scholarships.index') }}" class="btn btn-secondary ml-2">Batal</a>
      </form>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    ...
    const trixInitialValues = {
      content: {!! json_encode(old('content', $scholarship->content)) !!},
    };
