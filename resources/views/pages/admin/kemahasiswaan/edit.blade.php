...
<div class="container-fluid mb-4">
  ...
  <h1 class="mt-4">Edit Kemahasiswaan</h1>
  <ol class="breadcrumb mb-4">
      <li class="breadcrumb-item"><a href="{{ route('portal-admin.kemahasiswaan.index') }}">Daftar Kemahasiswaan</a></li>
      <li class="breadcrumb-item active">Edit Kemahasiswaan</li>
      <li class="breadcrumb-item active">{{ $kemahasiswaan->name }}</li>
  </ol>
  <div class="card">
    <div class="card-body">
        <form action="{{ route('portal-admin.kemahasiswaan.update', $kemahasiswaan->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          <div class="form-group">
              <label for="name">Nama</label>
              <input autocomplete="off" type="text" class="form-control" id="name" name="name" value="{{ $kemahasiswaan->name }}" required>
          </div>
          <div class="card mb-3">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" placeholder="Contoh: https://example.com atau konseling (tanpa link, kecil)" id="link" name="link" value="{{ $kemahasiswaan->link }}">
          </div>
          <button type="submit" class="btn btn-primary">Update</button>
          <a href="{{ route('portal-admin.kemahasiswaan.index') }}" class="btn btn-secondary ml-2">Batal</a>
      </form>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    ...
    const trixInitialValues = {
      description: {!! json_encode(old('description', $kemahasiswaan->description)) !!},
    };
